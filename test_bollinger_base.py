#!/usr/bin/env python3
"""
Test Base Bollinger Bands Signal with Common Filtering Framework
================================================================

Base Signal: 25-period, 2.0 std Bollinger breakout
- Buy: Price closes above upper band (breakout)
- Sell: Price closes below lower band (breakdown)
...
print("=" * 70)
print("TEST BASE BOLLINGER BANDS SIGNAL — COMMON FILTERING FRAMEWORK")
print("=" * 70)
...
# ================================================================
# Layer 1: Bollinger Bands Base Signal
# ================================================================
print("\n[3/6] Computing Bollinger Bands base signal...")

# Bollinger parameters: 25-period, 2.0 std
BB_PERIOD = 25
BB_STD = 2.0

df['bb_mid'] = sma(df['close'], BB_PERIOD)
df['bb_std'] = df['close'].rolling(BB_PERIOD).std()
df['bb_upper'] = df['bb_mid'] + BB_STD * df['bb_std']
df['bb_lower'] = df['bb_mid'] - BB_STD * df['bb_std']

# Base signal: Buy on upper band breakout, Sell on lower band breakdown
df['bb_buy'] = (df['close'] > df['bb_upper']).astype(float)
df['bb_sell'] = (df['close'] < df['bb_lower']).astype(float)

print(f"  Raw BB buy bars: {int(df['bb_buy'].sum())}")
print(f"  Raw BB sell bars: {int(df['bb_sell'].sum())}")
...
# Entry: Bollinger buy AND gates pass
df['entry_signal'] = df['bb_buy'] * gates_pass

# Exit: Bollinger sell (breakdown below lower band)
df['exit_signal'] = df['bb_sell']
...
# ================================================================
print("\n" + "=" * 70)
print("PERFORMANCE METRICS — BOLLINGER BANDS BASE")
print("=" * 70)

print(f"\n{'─'*50}")
print(f"  Signal:       Bollinger Bands ({BB_PERIOD}p, {BB_STD}σ breakout)")
...
print("\n" + "=" * 70)
print("TEST COMPLETE — BOLLINGER BANDS BASE")
print("=" * 70)

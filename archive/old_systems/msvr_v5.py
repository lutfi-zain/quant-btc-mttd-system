#!/usr/bin/env python3
"""
MSVR v5 — Selective Entry + Entropy Exit
==========================================

Changes from v4:
1. More selective entry (higher threshold for gates)
2. Entropy-based exit (Family 7: Entropy)
   - When entropy SPIKES → market becomes random → EXIT
   - More principled than RSI divergence
3. Optimized min_hold (25 days)

Key Insight:
Ichimoku has 11 trades because it's SELECTIVE.
We need to be more selective too!
"""
...
print("=" * 70)
print("MSVR v5 — Selective Entry + Entropy Exit")
print("=" * 70)
...
df['er_gate'] = (df['er'] > 0.30).astype(float)  # Higher threshold = more selective
...
df['entropy_entry'] = (df['entropy'] < 2.3).astype(float)  # More selective

# EXIT: Entropy SPIKE (market becoming random)
df['entropy_ma'] = df['entropy'].rolling(20).mean()
df['entropy_spike'] = ((df['entropy'] > df['entropy_ma'] * 1.3) & df['entropy'].notna() & df['entropy_ma'].notna()).astype(float)
...
# ================================================================
# EXIT SIGNAL — Entropy Spike
# ================================================================
print("\n[4/6] Computing exit signal (Entropy Spike)...")

# Exit when:
# 1. Entropy spikes (market becomes random)
# 2. OR Cycle phase reverses
# 3. OR MSVR flips

df['cycle_exit'] = (df['cycle_signal'] < -0.5).astype(float)
df['msvr_exit'] = (df['msvr_signal'] < -0.1).astype(float)
...
# ================================================================
# POSITION — Selective Entry + Entropy Exit
# ================================================================
print("\n[5/6] Applying selective entry + entropy exit...")

def apply_selective_position(entry_signal, exit_signal, min_hold=25, max_hold=60):
    """
    Selective entry + entropy exit.
    - Entry: ALL signals must agree
    - Exit: Entropy spike OR cycle reversal OR MSVR flip
    - Min hold: 25 days
    - Max hold: 60 days
    """
    ...
    for i in range(len(result)):
        if entry_signal.iloc[i] == 1.0 and not in_position:
            ...
            if hold_count >= min_hold and exit_signal.iloc[i] == 1.0:
                # Exit: min_hold satisfied + exit signal
                in_position = False
                hold_count = 0
                result.iloc[i] = 0.0
...
min_hold_values = [15, 20, 25, 30, 35]
results = []

for mh in min_hold_values:
    df['position'] = apply_selective_position(
        df['entry_signal'], 
        df['exit_signal'], 
        min_hold=mh, 
        max_hold=60
...
# ================================================================
print("\n" + "=" * 70)
print("MSVR v5 RESULTS — Selective Entry + Entropy Exit")
print("=" * 70)
...
print(f"{'MSVR v5 (Best)':<25} {best_sharpe['trades']:<10} {best_sharpe['win_rate']}%{'':<7} {best_sharpe['sharpe']:<10} {best_sharpe['cagr']}%{'':<7}")

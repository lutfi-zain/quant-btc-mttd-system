#!/usr/bin/env python3
"""
MSVR v7 — Medium-Term Target (25-35 trades)
=============================================

New Target:
- Trades: 25-35 (medium-term, 1 trade every 3-4 months)
- Win Rate: >60%
- Sharpe: >1.35
- CAGR: >50%

Changes from v6:
1. Relaxed entry: 4 of 5 gates (not ALL 5)
2. Optimized parameters for 25-35 trades
3. Extended hold: 60-90 days

Statistical Families: 10
"""
...
print("=" * 70)
print("MSVR v7 — Medium-Term Target (25-35 trades)")
print("=" * 70)
...
df['er_gate'] = (df['er'] > 0.25).astype(float)  # Relaxed from 0.30
...
df['entropy_gate'] = (df['entropy'] < 2.5).astype(float)  # Relaxed from 2.3
...
# ================================================================
# COMPOSITE SIGNAL — RELAXED ENTRY
# ================================================================
print("\n[3/6] Generating relaxed entry signal...")
...
# Gates: 4 of 5 must pass (relaxed!)
gate_signal = (
    df['er_gate'] + 
    df['vol_direction'] + 
    df['entropy_gate'] + 
...
gates_pass = (gate_signal >= 4).astype(float)  # 4 of 5 (relaxed from ALL 5)

# Entry: Core × Gates
df['entry_signal'] = core_signal * gates_pass

# ================================================================
# EXIT SIGNAL — Entropy Spike + Cycle Reversal
# ================================================================
print("\n[4/6] Computing exit signal...")

df['cycle_exit'] = (df['cycle_signal'] < -0.3).astype(float)
df['msvr_exit'] = (df['msvr_signal'] < -0.05).astype(float)
...
# ================================================================
# POSITION — Medium-Term Target
# ================================================================
print("\n[5/6] Applying medium-term position sizing...")

def apply_medium_term_position(entry_signal, exit_signal, min_hold=30, max_hold=90):
    """
    Medium-term position sizing.
    - Entry: Core × 4/5 gates
...
configs = [
    {'min_hold': 25, 'max_hold': 75, 'gates_required': 4},
    {'min_hold': 30, 'max_hold': 90, 'gates_required': 4},
    {'min_hold': 35, 'max_hold': 105, 'gates_required': 4},
    {'min_hold': 30, 'max_hold': 90, 'gates_required': 3},  # Even more relaxed
]
...
for cfg in configs:
    ...
    df['position'] = apply_medium_term_position(
        entry_signal_cfg, 
        df['exit_signal'], 
...
# ================================================================
print("\n" + "=" * 70)
print("MSVR v7 RESULTS — Medium-Term Target (25-35 trades)")
print("=" * 70)
...
if target_results:
    best = best_sharpe
    print(f"{'MSVR v7 (Best)':<25} {best['trades']:<10} {best['win_rate']}%{'':<7} {best['sharpe']:<10} {best['cagr']}%{'':<7} {best['avg_hold']}d{'':<7}")

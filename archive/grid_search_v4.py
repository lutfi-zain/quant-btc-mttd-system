#!/usr/bin/env python3
"""
MTTD Grid Search V4 — DIVERSIFIED INDICATOR SET
=================================================

Uses 6 indicators from DIFFERENT statistical families:
1. Adaptive Regime Cloud (Fractal/Hurst) - Regime detection
2. ALMA Lag (Smoothing) - Trend direction
3. Fourier For Loop (Spectral) - Cycle detection
4. Z-Score Adaptive Oscillator (Entropy/Statistics) - Mean-reversion
5. IRS Elder Force Volume Index (Volume) - Conviction confirmation
6. Kalman RSI (Bayesian) - Momentum (kept for robustness)

Key difference from V3: GENUINE factor diversification across statistical families.
...
print("=" * 70)
print("MTTD GRID SEARCH V4 — DIVERSIFIED INDICATOR SET")
print("=" * 70)
print("Indicators: 6 (Fractal, Smoothing, Spectral, Entropy, Volume, Bayesian)")
...
# ================================================================
# DIVERSIFIED Indicator Definitions (6 indicators, 6 families)
# ================================================================
print("\n[3/6] Defining indicator search space (6 indicators, 6 statistical families)...")

INDICATORS = [
    # Family 5: Fractal — Regime detection
    {
        ...
        'family': 'Fractal',
        'role': 'Regime gate — determines market context'
    },
    # Family 1: Smoothing — Trend direction
    {
        ...
        'family': 'Smoothing',
        'role': 'Primary trend signal — direction and conviction'
    },
    # Family 4: Spectral — Cycle detection (NEW)
    {
        'name': 'fourier_for_loop',
        'category': 'oscillator',
        'func_name': 'fourier_for_loop',
        'params': {'n': [1, 2, 3], 'start': [1, 5], 'end': [30, 45]},
        'family': 'Spectral',
        'role': 'Cycle detection — identifies dominant cycle periods'
    },
    # Family 7: Entropy/Statistics — Mean-reversion (NEW)
    {
        'name': 'z_score_adaptive_oscillator_suite',
        'category': 'oscillator',
        'func_name': 'z_score_adaptive_oscillator_suite',
        'params': {'z_score_len': [10, 14, 20], 'threshold': [1.0, 1.5, 2.0]},
        'family': 'Entropy',
        'role': 'Mean-reversion — statistical outlier detection'
    },
    # Volume — Conviction confirmation (NEW)
    {
        'name': 'irs_elder_force_volume_index',
        'category': 'perpetual',
        'func_name': 'irs_elder_force_volume_index',
        'params': {'length': [30, 40, 50]},
        'family': 'Volume',
        'role': 'Volume confirmation — conviction behind price moves'
    },
    # Family 9: Bayesian — Momentum (kept for robustness)
    {
        ...
        'family': 'Bayesian',
        'role': 'Momentum confirmation — noise-filtered RSI'
    }
]
...
for ind in INDICATORS:
    ...
    total_combos *= combos
    print(f"    {ind['name']}")
    print(f"      Family: {ind['family']} | Role: {ind['role']}")
    print(f"      Params: {list(ind['params'].keys())} → {combos} combos")
...
for ind_def in INDICATORS:
    ...
    print(f"\n  {ind_name} ({ind_def['family']}): {len(combinations)} combinations")
...
for ind_def in INDICATORS:
    ...
    if signal is not None:
        ...
        print(f"    ✓ {ind_name} ({ind_def['family']})")
    else:
...
if n_indicators > 1:
    ...
    for col in corr_matrix.columns:
        print(f"{col[:15]:>16s}", end='')
    print()
    for idx in corr_matrix.index:
        print(f"  {idx:30s}", end='')
        for col in corr_matrix.columns:
            print(f"{corr_matrix.loc[idx, col]:16.2f}", end='')
        print()
    ...
    if high_corr_pairs:
        ...
        for ind1, ind2, corr in high_corr_pairs:
            print(f"    {ind1} ↔ {ind2}: {corr:.2f}")
    else:
...
# Save results
output = {
    'version': 'v4_diversified_6_indicators',
    ...
    'indicators_used': [
        {'name': ind['name'], 'family': ind['family'], 'role': ind['role']}
        for ind in INDICATORS
    ],
...
output_path = os.path.join(project_root, 'grid_search_v4_results.json')
with open(output_path, 'w') as f:
    json.dump(output, f, indent=2, default=str)

print(f"\nResults saved to: {output_path}")
print("\n" + "=" * 70)
print("GRID SEARCH V4 COMPLETE — DIVERSIFIED INDICATORS")
print("=" * 70)

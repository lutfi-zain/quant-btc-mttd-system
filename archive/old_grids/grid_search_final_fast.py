#!/usr/bin/env python3
"""
Final Fast Grid Search — Optimized for Speed
=============================================

Pre-computes all features once, then runs many param combos quickly.
Key optimization: compute cycle phase on full dataset, not per split.
"""
...
def compute_cycle_phase_fast(df: pd.DataFrame, lookback: int = 40) -> pd.Series:
    """Vectorized cycle phase computation."""
    ...
    # Pre-compute Hann window and FFT plans
    hann = np.hanning(lookback)

    # Compute in bulk for all valid start positions
    src_vals = src.values
    for i in range(lookback - 1, n):
        window = src_vals[i - lookback + 1:i + 1]
        if np.any(np.isnan(window)):
            continue
        windowed = (window - np.mean(window)) * hann
...
def generate_all_features(df: pd.DataFrame) -> pd.DataFrame:
    """Generate all features in one pass."""
    df = df.copy()
    ...
    phase = compute_cycle_phase_fast(df, lookback=40)
...
def ichimoku_signal_fast(df: pd.DataFrame, idx_start: int, idx_end: int,
                         min_hold: int, max_hold: int,
                         profit_take_pct: float = 0.10,
                         trailing_stop_pct: float = 0.05) -> np.ndarray:
    """Fast Ichimoku signal with profit-taking and trailing stop. Returns numpy array."""
    n = idx_end - idx_start
    position = np.zeros(n)
    in_position = False
    hold_count = 0
    ...
    imo_arr = df['IMO'].values
    er_arr = df['ER'].values
    std_arr = df['IMO_Std'].values
    ent_arr = df['Entropy'].values
    close_arr = df['close'].values
    cloud_min_arr = df['cloud_min'].values
...
def apply_gate_fast(ichimoku_pos: np.ndarray, filter_signals: list, gate_threshold: int) -> np.ndarray:
    """Fast gate application. filter_signals is list of numpy arrays."""
    n = len(ichimoku_pos)
    result = np.zeros(n)
...
def compute_metrics_fast(signal: np.ndarray, prices: np.ndarray) -> dict:
    """Fast metrics computation from numpy arrays."""
    n = len(prices)
    returns = np.diff(prices) / prices[:-1]
    
    # Signal shifted by 1 (enter next day)
    strat_returns = np.zeros(n - 1)
    strat_returns[1:] = returns * signal[:-1]  # signal at t-1 applied to returns at t
    ...
    if np.sum(np.abs(strat_returns)) == 0:
...
def run_grid_search():
    print("=" * 70)
    print("FINAL FAST GRID SEARCH — PROFIT-TAKING + TRAILING STOP")
    print("=" * 70)
    ...
    print("  Pre-computing all features (one-time)...")
    ...
    prices_all = df_feat['close'].values
    prices_train = df['close'].values[train_start_idx:train_end_idx]
    prices_test = df['close'].values[test_start_idx:test_end_idx]
    
    print(f"  Train: {train_start_idx}-{train_end_idx} ({train_end_idx - train_start_idx} bars)")
    print(f"  Test:  {test_start_idx}-{test_end_idx} ({test_end_idx - test_start_idx} bars)")
    
    # Filter column names and pre-extract arrays
    filter_cols = ['msvr_direction', 'smooth_direction', 'cycle_direction', 'entropy_gate']
    filter_arrays_train = [df_feat[col].values[train_start_idx:train_end_idx] for col in filter_cols]
    filter_arrays_test = [df_feat[col].values[test_start_idx:test_end_idx] for col in filter_cols]
                    ...
                    for ts in trailing_stops:
                        ...
                        try:
                            ...
                            ichimoku_train = ichimoku_signal_fast(
                                df_feat, train_start_idx, train_end_idx, mh, xh, pt, ts)
                            ichimoku_test = ichimoku_signal_fast(
                                df_feat, test_start_idx, test_end_idx, mh, xh, pt, ts)
    ...
    df_results['score'] = df_results.apply(score, axis=1)

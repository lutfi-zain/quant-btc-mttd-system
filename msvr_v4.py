#!/usr/bin/env python3
"""
MSVR v4 — Relaxed Min-Hold + Exit Principle
=============================================

Changes from v3:
1. Relaxed min_hold: 10-30 days (was 45)
2. New Exit Principle: Momentum Divergence (Family 5: Fractal)
   - When price makes new high but RSI doesn't = EXIT
   - Protects profits by exiting before reversal

Statistical Families:
1. MSVR (Smoothing) — Direction
2. SuperSmoother (Filtering) — Noise reduction
3. LinearReg (Regression) — Trend
4. Cycle Phase (Spectral) — Timing
5. Efficiency Ratio (Fractal) — Trend strength
6. Volatility Cluster (GARCH) — Risk management
7. Shannon Entropy (Entropy) — Randomness filter
8. Volume Confirm (Volume) — Confirmation
9. HMM Regime (Bayesian) — Regime filter
10. Momentum Divergence (Fractal) — EXIT signal ← NEW!
"""
...
print("=" * 70)
print("MSVR v4 — Relaxed Min-Hold + Exit Principle")
print("=" * 70)
...
df['er_gate'] = (df['er'] > 0.25).astype(float)
...
df['entropy_gate'] = (df['entropy'] < 2.5).astype(float)
...
# ================================================================
# Layer 10: Momentum Divergence (Family 5: Fractal) — EXIT SIGNAL
# ================================================================
print("\n[3/6] Computing exit signal (Momentum Divergence)...")
...
# Momentum Divergence: Price new high but RSI not = EXIT
df['price_high_20'] = df['close'].rolling(20).max()
df['rsi_high_20'] = df['rsi'].rolling(20).max()

# Divergence: Price at new high BUT RSI NOT at new high
df['bearish_divergence'] = (
    (df['close'] >= df['price_high_20'] * 0.98) &  # Price near 20-day high
    (df['rsi'] < df['rsi_high_20'] * 0.95)          # RSI NOT at new high
).astype(float)

# Also exit when RSI overbought + divergence
df['exit_signal'] = (
...
# ================================================================
# COMPOSITE SIGNAL with RELAXED MIN-HOLD
# ================================================================
print("\n[4/6] Generating composite signal...")

# Entry signal: All core principles agree
core_signal = (
    df['msvr_direction'] * 
    df['smooth_direction'] * 
    df['lr_direction'] * 
...
# Gate filters (at least 3 of 5 must pass)
gate_signal = (
    df['er_gate'] + 
    df['vol_direction'] + 
    df['entropy_gate'] + 
...
gates_pass = (gate_signal >= 3).astype(float)

# Entry: Core × Gates
df['entry_signal'] = core_signal * gates_pass

# ================================================================
# RELAXED MIN-HOLD with EXIT SIGNAL
# ================================================================
print("\n[5/6] Applying relaxed min-hold + exit signal...")

def apply_relaxed_hold_with_exit(entry_signal, exit_signal, min_hold=15, max_hold=60):
    """
    Relaxed min-hold with exit signal.
    - Can exit early if exit_signal fires
    - But must hold at least min_hold days
    - Must exit after max_hold days
    """
    ...
    for i in range(len(result)):
        if result.iloc[i] == 1.0 and not in_position:
            ...
            if hold_count >= min_hold:
                ...
                # Must hold (min_hold not reached)
                result.iloc[i] = 1.0
                hold_count += 1
        elif in_position:
            # Still in position
            if exit_signal.iloc[i] == 1.0 and hold_count >= min_hold:
                # Exit signal + min_hold satisfied
                result.iloc[i] = 0.0
                in_position = False
                hold_count = 0
            elif hold_count >= max_hold:
...
# Test different min_hold values
min_hold_values = [10, 15, 20, 25, 30]
results = []

for mh in min_hold_values:
    df['position'] = apply_relaxed_hold_with_exit(
        df['entry_signal'], 
        df['exit_signal'], 
        min_hold=mh, 
        max_hold=60
...
# ================================================================
print("\n" + "=" * 70)
print("MSVR v4 RESULTS — Relaxed Min-Hold + Exit Signal")
print("=" * 70)

print(f"\n{'MinHold':<10} {'Trades':<10} {'WinRate':<10} {'Sharpe':<10} {'CAGR':<10} {'MaxDD':<10} {'InMkt%':<10}")
print("-" * 70)

for r in results:
    print(f"{r['min_hold']:<10} {r['trades']:<10} {r['win_rate']:<10} {r['sharpe']:<10} {r['cagr']:<10} {r['max_dd']:<10} {r['in_market']:<10}")
...
print("BEST CONFIGS:")
print(f"{'='*70}")
print(f"Best Sharpe:    MH={best_sharpe['min_hold']}, Sharpe={best_sharpe['sharpe']}, WinRate={best_sharpe['win_rate']}%, CAGR={best_sharpe['cagr']}%")
print(f"Best WinRate:   MH={best_winrate['min_hold']}, Sharpe={best_winrate['sharpe']}, WinRate={best_winrate['win_rate']}%, CAGR={best_winrate['cagr']}%")
print(f"Best CAGR:      MH={best_cagr['min_hold']}, Sharpe={best_cagr['sharpe']}, WinRate={best_cagr['win_rate']}%, CAGR={best_cagr['cagr']}%")

# Compare with Ichimoku
print(f"\n{'='*70}")
print("COMPARISON WITH ICHIMOKU:")
print(f"{'='*70}")
print(f"{'Config':<25} {'Trades':<10} {'WinRate':<10} {'Sharpe':<10} {'CAGR':<10}")
print("-" * 65)
print(f"{'Ichimoku':<25} {'11':<10} {'63.6%':<10} {'1.31':<10} {'55.6%':<10}")
print(f"{'MSVR v3 (MH=45)':<25} {'15':<10} {'66.7%':<10} {'1.12':<10} {'39.5%':<10}")
print(f"{'MSVR v4 (Best)':<25} {best_sharpe['trades']:<10} {best_sharpe['win_rate']}%{'':<7} {best_sharpe['sharpe']:<10} {best_sharpe['cagr']}%{'':<7}")

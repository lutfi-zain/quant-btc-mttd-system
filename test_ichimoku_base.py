#!/usr/bin/env python3
"""
Test Base Ichimoku Signal with Common Filtering Framework
==========================================================

Base Signal: Tenkan/Kijun crossover + Cloud position
- Buy: Tenkan > Kijun AND price > Cloud (bullish cloud)
- Sell: Tenkan < Kijun OR price < Cloud (bearish cloud)
...
print("=" * 70)
print("TEST BASE ICHIMOKU SIGNAL — COMMON FILTERING FRAMEWORK")
print("=" * 70)
...
# ================================================================
print("\n[3/6] Computing Ichimoku base signal...")

# Ichimoku parameters (crypto-calibrated)
p1 = 20   # Tenkan period
p2 = 60   # Kijun period
p3 = 120  # Senkou B period
...
# Cloud (Senkou Spans)
df['senkou_a_raw'] = (df['tenkan_sen'] + df['kijun_sen']) / 2
df['senkou_b_raw'] = (df['high'].rolling(p3).max() + df['low'].rolling(p3).min()) / 2
df['senkou_a'] = df['senkou_a_raw'].shift(p2)
df['senkou_b'] = df['senkou_b_raw'].shift(p2)

# Cloud boundaries
df['cloud_max'] = np.maximum(df['senkou_a'], df['senkou_b'])
df['cloud_min'] = np.minimum(df['senkou_a'], df['senkou_b'])

# Ichimoku base signal
# Buy: Tenkan > Kijun AND price > cloud_max
# Sell: Tenkan < Kijun OR price < cloud_min
df['ichimoku_buy'] = (
...
print(f"  Raw Ichimoku buy bars: {int(df['ichimoku_buy'].sum())}")
print(f"  Raw Ichimoku sell bars: {int(df['ichimoku_sell'].sum())}")
...
# Exit: Ichimoku sell (immediate exit on sell signal)
df['exit_signal'] = df['ichimoku_sell']
...
# ================================================================
print("\n" + "=" * 70)
print("PERFORMANCE METRICS — ICHIMOKU BASE")
print("=" * 70)

print(f"\n{'─'*50}")
print(f"  Signal:       Ichimoku (Tenkan/Kijun + Cloud)")
...
print("\n" + "=" * 70)
print("TEST COMPLETE — ICHIMOKU BASE")
print("=" * 70)
